...
<body>
	<!-- 다른 JSP 파일의 내용을 삽입 -->
	<!-- 1. %@ include 는 외부 JSP 파일의 내용을 현재 위치에 복사해서 넣은 후 하나의 JSP 파일로 실행(변수 공유 O, 같은 프로젝트에 속한 것만) -->
	<%@ include file="/WEB-INF/views/comm/menu.jsp" %> <!-- 제일 기본적인 방법 -->
	<!-- 2. jsp:include 는 별도의 독립된 JSP 파일로 요청을 보내서 출력 결과를 현재 위치에 포함(변수 공유 X, 같은 프로젝트에 속한 것만) -->
	<%-- <jsp:include page="/WEB-INF/views/comm/menu.jsp"></jsp:include> --%>
	<!-- 3. c:import 는 다른 웹 사이트(서비스)의 자원도 포함 가능(외부에 있는 것까지 요청을 보내서 넣을 수 있음) -->
